...
class EvolutionMinAffectionViewHolder(parent: ViewGroup) :
    BaseViewHolder<EvolutionParam.MinAffection>(parent, R.layout.li_evolution_detail_text) {
    private val binding: LiEvolutionDetailTextBinding by viewBinding()
    override fun bind(item: EvolutionParam.MinAffection) {
        super.bind(item)
        binding.name.setText(StringDesc.StringR(R.string.evolution_affection_template, arrayOf(item.value)))
    }
}