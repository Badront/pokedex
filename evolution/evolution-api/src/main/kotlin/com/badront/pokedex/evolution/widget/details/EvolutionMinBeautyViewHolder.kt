...
class EvolutionMinBeautyViewHolder(parent: ViewGroup) :
    BaseViewHolder<EvolutionParam.MinBeauty>(parent, R.layout.li_evolution_detail_text) {
    private val binding: LiEvolutionDetailTextBinding by viewBinding()
    override fun bind(item: EvolutionParam.MinBeauty) {
        super.bind(item)
        binding.name.setText(StringDesc.StringR(R.string.evolution_beauty_template, arrayOf(item.value)))
    }
}